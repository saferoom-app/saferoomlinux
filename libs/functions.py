# Import section
import datetime,hashlib,binascii,rncryptor,base64, math, os, safeglobals,json, shutil
...
def getMime(fileName):
    ...
    extension = array[-1]
    try:
        return safeglobals.MIME_TYPES[extension]
    except KeyError:
        return safeglobals.MIME_DEFAULT
...
def write_status(guid,status):
    ...
    with open(os.path.join(safeglobals.path_tmp,guid+".status"),"w") as f:
...
def get_status(guid):
    ...
    status = {}
    try:
        with open(os.path.join(safeglobals.path_tmp,guid+".status"),"r") as f:
...
def remove_status(guid):
    try:
        os.remove(os.path.join(safeglobals.path_tmp,guid+".status"))
    except Exception as e:
        pass

def is_backed_up(guid):
    return os.path.exists(safeglobals.path_notes_backup % guid)