...
def list_favourites():
	...
	if os.path.isfile(config.path_favourites) == False:
		return favourites

	# Loading a list of favourites
	with open(config.path_favourites,"r") as f:
...
def add_to_favourites(item):
	...
	with open(config.path_favourites,"w") as f:
...
def list_quick_links():
	...
	links = []
	try:
		with open(config.path_quicklinks,"r") as f:
...
def save_quick_links(links):
	...
	try:
		with open(config.path_quicklinks,"w") as f:
